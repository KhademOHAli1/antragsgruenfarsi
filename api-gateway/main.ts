...
const CONFIG = {
  ...
  antragsgruenBaseUrl: env.ANTRAGSGRUEN_BASE_URL ||
    Deno.env.get("ANTRAGSGRUEN_BASE_URL") || "http://localhost:8080",
  rarimoIssuer: env.RARIMO_ISSUER || Deno.env.get("RARIMO_ISSUER") ||
    "https://auth.rarimo.com",
  jwtAudience: env.JWT_AUDIENCE || Deno.env.get("JWT_AUDIENCE") ||
    "antragsgruen-api",
...
const JWKS = createRemoteJWKSet(
  new URL(`${CONFIG.rarimoIssuer}/.well-known/jwks.json`),
);
...
async function authMiddleware(ctx: Context, next: () => Promise<unknown>) {
  ...
  try {
    ...
    logger.debug(
      `Authenticated user: ${payload.sub} with scope: ${payload.scope}`,
    );
...
function requireScope(requiredScope: string) {
  return async (ctx: Context, next: () => Promise<unknown>) => {
    ...
    if (!hasScope) {
      logger.warn(
        `Access denied: user ${user.sub} lacks scope ${requiredScope}`,
      );
...
async function proxyToAntragsgruen(ctx: Context, path: string) {
  ...
  for (const [key, value] of ctx.request.headers) {
    if (
      !["host", "connection", "transfer-encoding", "upgrade", "authorization"]
  ...
  try {
    ...
    for (const [key, value] of response.headers) {
      if (
        !["connection", "transfer-encoding", "upgrade"].includes(
          key.toLowerCase(),
        )
      ) {
